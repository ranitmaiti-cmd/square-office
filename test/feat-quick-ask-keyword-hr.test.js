...
(async () => {
  ...
  check('renderLeaves() unchanged from main except the V30 isSystemFlagged Cancel-button guard', (() => {
    const cur = extractFunction(fullScript, 'renderLeaves').src;
    const main = extractFunction(mainFullScript, 'renderLeaves').src;
  ...
  check('renderApprovals() unchanged from main except the V30 system-flagged label/autoDeducted-on-approve additions', (() => {
    const cur = extractFunction(fullScript, 'renderApprovals').src;
    const main = extractFunction(mainFullScript, 'renderApprovals').src;
