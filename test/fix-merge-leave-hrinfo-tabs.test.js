...
(async () => {
  ...
  // contains the isSystemFlagged additions to both functions -- a "strip
  // the V30 addition out of this branch, then compare to main" check
  // would now be comparing against main's OWN former self and false-fail
  // forever. Same structural fix already applied elsewhere in this
  ...
  check('renderLeaves() is byte-for-byte unchanged from main (modulo V34 entitlement denominators)', flatEnt(extractFunction(fullScript, 'renderLeaves')) === flatEnt(extractFunction(mainFullScript, 'renderLeaves')));
  ...
  check('renderApprovals() (approve/reject handlers, balance decrement on approval) is byte-for-byte unchanged from main', extractFunction(fullScript, 'renderApprovals') === extractFunction(mainFullScript, 'renderApprovals'));
